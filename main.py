...
@register("astrbot_plugin_tool_prompts", "PluginDeveloper", "一个LLM工具调用和媒体链接处理插件", "0.0.4")
class ToolCallNotifierPlugin(Star):
    ...
    async def on_llm_response_handler(self, event: AstrMessageEvent, resp: LLMResponse):
        ...
        if resp.role == "assistant" and resp.completion_text:
            ...
            if not matches:
                logger.debug("LLM响应处理器：未找到媒体链接，不进行特殊处理。")
                return # 没有链接，正常发送

            # 找到了链接，我们将接管消息发送，并清空原始响应文本
            logger.info("LLM响应处理器：找到媒体链接，将分条发送并阻止原始消息。")
            ...
            for match in matches:
                ...
                logger.info(f"LLM响应处理器：匹配到链接: {url}")
            ...
            # 清空原始响应，防止重复发送
            logger.info("LLM响应处理器：清空原始响应文本以防止重复发送。")
    ...
    def _create_media_segment(self, path_or_url: str):
        ...
        if not (is_url or is_file):
            logger.warning(f"媒体处理：路径 '{path_or_url}' 既不是有效URL也不是本地文件。")
